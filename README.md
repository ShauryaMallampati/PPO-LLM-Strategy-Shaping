...
PPO in Overcooked-AI with binary reward shaping from a frozen LLM evaluator, plus post-training unilateral-deviation diagnostics.
...
| [02_training.ipynb](notebooks/02_training.ipynb) | Train the main baselines on `asymmetric_advantages`; PPO+LLM uses binary reward shaping from a frozen LLM evaluator |
...
| [07_llm_sensitivity.ipynb](notebooks/07_llm_sensitivity.ipynb) | Sensitivity sweep for PPO+LLM with a smaller frozen LLM evaluator (GPT-Neo 125M) |
...
- PPO+LLM: PPO with binary reward shaping from a frozen GPT-Neo evaluator
- CC_PPO: centralized-critic PPO
- SP_PPO: self-play PPO
- HARL: hand-designed shaping baseline retained under the legacy HARL name
...
- Frozen LLM evaluator shaping bonus: `LLM_BONUS = 0.2`
...
- The method uses binary reward shaping from a frozen LLM evaluator: during PPO training, the frozen language model contributes a fixed `+0.2` bonus when the prompt-conditioned comparison favors `good` over `bad`.
